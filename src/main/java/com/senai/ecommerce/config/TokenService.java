...
public class TokenService {
	...
	public String GenerateToken(Usuario user) {
		try {
			...
			String Token = JWT.create().withIssuer("auth-rest-api").withSubject(user.getEmail())
					.withExpiresAt(GenExpDate()).sign(algorithm);
			return Token;
	...
	public String ValidatToken(String Token) {
		try {
			Algorithm algorithm = Algorithm.HMAC256(secret);
			return JWT.require(algorithm).withIssuer("auth-rest-api").build().verify(Token).getSubject();
